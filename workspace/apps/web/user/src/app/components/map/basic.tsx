...
const Map = ({ center: posix, raw, zoom = defaults.zoom }: MapProps) => {
  return (
    <MapContainer center={posix} zoom={zoom}
  ...
  useEffect(() => {
    ...
    if (map) {
      ...
      routes.forEach((route: any) => {
        const layers = route.attributes.layers.data;
        layers.forEach((layer: any) => {
          const layerTitle = layer.attributes.title;
          const sortKey = layer.attributes.sortKey || "";
          const visible = layer.attributes.visible as boolean;

          if (!rawOverlay[layerTitle]) {
            rawOverlay[layerTitle] = { layer: new L.LayerGroup(), sortKey, visible }
          }
          if (visible) {
        ...
        if (route.attributes.makeNameLayer) {
          const sortKey = route.attributes.sortKey
          rawOverlay[route.attributes.name] = { layer: new L.LayerGroup(), sortKey: sortKey, visible: true }
          drawRoute(route, rawOverlay[route.attributes.name].layer);
        }
      });
...
function drawWayPointRoute(data: any, layer: L.LayerGroup<any>) {
  const gpxString = data.attributes.gpx
  const textcolor = data.attributes.color
  const name = data.attributes.name
  const description = data.attributes.description
  const svgPin = data.attributes.startLocation?.svgPin

  if (!data.attributes.startLocation) {
  ...
  const coordinates: { lat: number, lon: number, wname: string, wcmt: string, wdesc: string }[] = [];
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    const startupPopup = L.popup({})
      .setLatLng([lat, lon])
      .setContent(`<div class='text-lg'>${wname}<br/>${name}</div><div>${wdesc}</div><div><a class="text-xs" href="${googleLink}" rel="noreferrer" target="_blank">Google Map</a></div>`)
...
function drawRoute(data: any, layer: L.LayerGroup<any>) {
  ...
  const name = data.attributes.name
  const opacity = data.attributes.opacity
  const weight = data.attributes.weight
  const startLocation = data.attributes.startLocation
  const description = data.attributes.description

  const polylineCoordinates: [number, number][] = parseGPX(gpxString);

  if (polylineCoordinates.length === 0) {
    drawWayPointRoute(data, layer);
  }

  const polyline = L.polyline(polylineCoordinates, { color, opacity, weight, lineCap: 'round' });
  ...
  if (startLocation) {
    const startPin = L.divIcon({
      className: "leaflet-data-marker",
      html: L.Util.template(startLocation.svgPin ?? iconSettings.mapIconUrl, iconSettings),
      iconAnchor: [12, 32],
      iconSize: [25, 30],
      popupAnchor: [0, -28]
    })
    ...
    const startupPopup = L.popup({})
      ...
      .setContent(`<div class='text-lg'>${name}</div><div>${description}</div><div>`)
