...
import { Avatar, Button, Dropdown, DropdownItem, DropdownMenu, DropdownSection, DropdownTrigger, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, User, useDisclosure } from "@nextui-org/react";
...
const UserDropDown = (params: any) => {
  ...
  const { isOpen: isQROpen, onOpen: openQR, onClose: closeQR } = useDisclosure();
  ...
  async function handleChangeTheme(event: React.ChangeEvent<HTMLSelectElement>) {
  ...
  return (
    <>
      ...
      >
        <DropdownTrigger>
          <Avatar
            src={session.user.image ?? "https://assets.cms.defcon.run/dcjack_4ad07c2493.svg"}
            ignoreFallback={true}
            size="lg"
          />
        </DropdownTrigger>
        ...
        >
          <DropdownSection aria-label="Profile & Actions" showDivider>
            ...
            <DropdownItem
              ...
              key="dashboard" className="gap-2 opacity-100"
              textValue="Dashboard">
            ...
            <DropdownItem
              startContent={
                <>
                  <Link replace={true} href={`/participation?show=${session.user.hasStrava ? "strava" : "manual"}`}>
            ...
            {!session.user.hasStrava ?
              <DropdownItem
                startContent={<FaStrava color="red" size={24} className={iconClasses} />}
                onClick={() => signIn("strava")}
                textValue="Link to Strava"
              >
                Link to Strava
              </DropdownItem> : <DropdownItem className="h-0 invisible" textValue="Already Strava'd" />}
          ...
          <DropdownItem
              ...
              key="showqr" className="gap-2 opacity-100"
              textValue="showqr"
              onClick={() => showQR()}
            >
              Display QR
...
function QRModal(isOpen: boolean, onClose: () => void, userDetail: any) {
  ...
  >
    <ModalContent>
      {() => (
        <>
          <ModalHeader className="flex flex-col gap-1 text-center">Unique QR Code</ModalHeader>
          <ModalBody>
            <img src={userDetail.eqr} />
          </ModalBody>
          <ModalFooter>
            <Button size="lg" color="danger" variant="light" onClick={closeWindow}>
              Done
            </Button>
          </ModalFooter>
        </>
