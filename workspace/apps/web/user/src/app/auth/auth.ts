...
import { EntityMapper, UpdateDiscord, UpdateGithub, UpdateNodeMailer, UpdateStrava } from "@defcon.run/web/db"
  ...
  callbacks: {
    ...
    async jwt({ token, account, profile, trigger, session }) {
      if (trigger === "update") {
        ...
        //TODO: DB Update with the new user preference

      } else if (account && profile) {
        if (account.provider === "discord") {
          token.name = `${profile.global_name}`
          token.picture = `${profile.image_url}`;
          await UpdateDiscord(token.email!, profile)
        } else if (account.provider === "github") {
          token.name = `${profile.login}`
          token.picture = `${profile.avatar_url}`
          await UpdateGithub(token.email!, profile)
        } else if (account.provider === "strava") {
          token.stravaId = `${profile.id}`
          token.name = `${profile.username}`
          token.picture = `${profile.profile_medium}`
          await UpdateStrava(token.email!, profile, account)
        }
        await setTokenFromDb()
      } else if (account && account.provider === "nodemailer") {
        await UpdateNodeMailer(token.email!)
        await setTokenFromDb()
      }

      async function setTokenFromDb() {
        const u = await entity.User.query(token?.email)

        if (u.Items?.length! > 0) {
          const user = u.Items?.at(0)
          token.email = user?.email
          token.userId = user?.id
          token.stravaId = user?.strava_account?.athlete?.id
          token.name = user?.name ?? token.name
          token.theme = user?.profile_theme ?? "dark"
