...
## AWS Architecture
The diagram shows two entries into the system - load balancer or CloudFront distrubtion. Only requests for `assets.cms.defon.run` are currently CloudFront'd. All other requests (eg. `app.defcon.run`) goto the load balancer. In the future, all traffic will flow through a CloudFront distribution.
